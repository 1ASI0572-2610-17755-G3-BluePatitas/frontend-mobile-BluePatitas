...
Administrator sign-up and shelter onboarding use backend endpoints. The admin flow is: create admin -> sign in -> create or recover shelter -> dashboard.

## Backend

Retrofit is configured with `BuildConfig.BACKEND_BASE_URL`. Production defaults to `https://backend-bluepatitas.onrender.com/`. For local backend testing from the Android emulator, set this in `local.properties`:
...
Debug builds allow cleartext traffic for local HTTP. Release builds do not enable cleartext globally.

`BluePatitasApi` currently exposes:
...
`RealAnimalRepository` consumes administrator animal endpoints and uploads selected animal images to the backend media endpoint. The backend owns Cloudinary integration; the mobile app only stores the returned URL in `photoUrl`. `RealMonitoringRepository` consumes `GET /api/monitoring/zones` and `GET /api/monitoring/alerts`, and owns local in-memory alerts generated by safe-zone simulation.
...
Firebase Push should replace or complement local notifications when real remote alert delivery is available. The UI and ViewModels should continue depending on use cases rather than API clients.
